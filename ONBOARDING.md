...
`python .\run_validation.py install-smoke`，它会调用 `test_distribution_smoke.py`
并在临时 venv 里分别验证
editable install、`pip install .` 和 wheel 安装三种形态，再确认统一 CLI 在
非仓库 `cwd` 下可用，且 `build-index` 默认不会把输出写回安装目录。
现在连 `build-index` 相关测试也已经切到仓库内 fixture：日常单测、
CLI parity 和安装态 smoke 都不需要外部 Obsidian Vault；只有你手动重建
真实 `section_page_index.json` 时才需要外部语料。
...
discrete-math-rag/
  ...
  _build_section_page_index_impl.py
                            # 索引构建实现
  run_validation.py         # 分层验证入口
  run_eval_suite.ps1        # 固定评测入口
  run_round.ps1             # 默认回归入口
  probe_queries.ps1         # probe 编排入口
  test_*.py                 # 契约与回归测试
  build_index_test_support.py
                            # build-index fixture 共享参数与期望载荷
  test_fixtures/build_index/
                            # build-index 测试夹具
...
| `run_validation.py` | 把快单测、install smoke、round、eval 分层成显式入口 |
...
| `test_fixtures/build_index/` | `build-index` 单测、CLI parity、安装态 smoke 的固定输入 | 仓库内文件 |
...
2. 你跑 `python -m discrete_math_rag build-index ...`。构建入口会读取外部
   ...
   写到当前工作目录，而不是安装目录。
3. 你跑 `test_build_section_page_index.py`、`test_cli.py` 或
   `test_distribution_smoke.py` 里的 `build-index` 覆盖。它们会显式把
   `test_fixtures/build_index/` 传给 `build-index`，用固定 fixture 复现构建
   契约，而不是依赖外部 Obsidian Vault。
...
- 先跑 `python .\run_validation.py quick`，再跑 `round`，最后跑 `eval`。这是这个仓库的默认验收顺序。
- `build-index` 相关测试已经 fixture 化；跑 `test_build_section_page_index.py`、
  `test_cli.py` 和 `test_distribution_smoke.py` 时不需要外部 Obsidian Vault。
- 改 CLI 之前先看 `test_cli.py`，它定义了“等价于旧脚本”到底是什么意思。
- 改打包或 CI 之前先跑 `python .\run_validation.py install-smoke`；它锁的是“editable /
